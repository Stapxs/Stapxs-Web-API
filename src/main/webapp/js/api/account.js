...
function loginAcc() {
    ...
    fetch("/acc/getKey/" + name)
        ...
        .then(data => {
            ...
            try {
                const httpRequest = new XMLHttpRequest();
                httpRequest.open("POST", "/acc/loginAcc", true);
                httpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                ...
                httpRequest.onreadystatechange = function () {
                    if (httpRequest.readyState === 4 && httpRequest.status === 200) {
                        ...
                        document.cookie = "id=" + id + "; expires=" + exp.toGMTString() + "; path=/";
                        document.cookie = "token=" + json.token + "; expires=" + exp.toGMTString() + "; path=/";
                        // 获取用户昵称
        ...
        .catch(function (e) {
            console.error(e)
            showErr(true, "获取信息失败！(3)")
        })

    return false
}

function showErr(isShow, info) {
    const mainCard = document.getElementById("pan-main")
    const card = document.getElementById("err-card")
    const txt = document.getElementById("err-info")
    if(isShow) {
        loading(false)
        console.log(getComputedStyle(mainCard, null).marginTop)
        if(getComputedStyle(mainCard, null).marginTop !== "0px") {
            mainCard.style.marginTop = "135px"
        }
        card.style.visibility = "visible"
        txt.innerText = info
    } else {
        if(getComputedStyle(mainCard, null).marginTop !== "0px") {
            mainCard.style.marginTop = "150px"
        }
